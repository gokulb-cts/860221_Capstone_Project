...
import com.cts.capstone.fms.constants.FmsUserConstants;
...
public class FmsUserRestController {
	...
	public Mono<ResponseEntity<FmsUser>> getUserByUserId(@PathVariable Long userId) {
		...
		return fmsUserService.getUserByUserId(userId)
								.map(event -> new ResponseEntity<FmsUser>(event, HttpStatus.OK))
								.defaultIfEmpty(new ResponseEntity<FmsUser>(HttpStatus.NOT_FOUND));
	}
	
	@GetMapping(value = USER_END_POINT + "/email/{emailId}", produces = MediaType.APPLICATION_STREAM_JSON_VALUE )
	public Mono<ResponseEntity<FmsUser>> getUserByEmailId(@PathVariable String emailId) {
		log.info("getUserByEmailId()");
		return fmsUserService.getUserByEmailId(emailId)
								.map(event -> new ResponseEntity<FmsUser>(event, HttpStatus.OK))
								.defaultIfEmpty(new ResponseEntity<FmsUser>(HttpStatus.NOT_FOUND));
	}
	...
	public Flux<ResponseEntity<FmsUser>> getUsersByRole(@PathVariable String roleName) {
		...
		return roleService.getRoleByRoleName(roleName)
						  .switchIfEmpty(Mono.error(new RoleNotFoundException(ROLE_NOT_FOUND)))
		 ...
		 */
			/*fmsUserService.getUsersByRole(role)
						.map(user -> new ResponseEntity<FmsUser>(user, HttpStatus.OK));*/
		//});
		//.defaultIfEmpty(Flux.just(new ResponseEntity<FmsUser>(HttpStatus.NOT_FOUND)));
	}
	...
	@PatchMapping(value = USER_END_POINT + "/{userId}/role/{roleName}", produces = MediaType.APPLICATION_STREAM_JSON_VALUE)
	public Mono<ResponseEntity<String>> assignUserRole(@PathVariable Long userId, @PathVariable String roleName) {
