...
public class FmsWebSecurity extends WebSecurityConfigurerAdapter {
	...
	public void configure(AuthenticationManagerBuilder auth) throws Exception {
		auth.userDetailsService(userService) /**service that implements UserDetailService which implements loadByUserName() where DB username and password are retrieved**/
			.passwordEncoder(bCryptPasswordEncoder); /**type of encoder used in order to
			 											encrypt incoming request password and compare
			 											with encrypted password from db**/
	}
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable() //Disable csrf for REST API
			.authorizeRequests()
			.antMatchers(HttpMethod.POST, SIGN_UP_URL) //for POST of this URL
			.permitAll() //permit without authentication
			.anyRequest() //for any other request
			.authenticated() //do authentication
			.and()
			.addFilter(getAuthenticaitonFilter()) //Add Authentication Filter
